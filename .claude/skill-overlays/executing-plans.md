...
- **C tasks:** dispatch the `gbdk-expert` agent with `"implement this task: <full task text>"` — it owns the TDD cycle, bank gates, build and the commit. It does **not** own `gb-c-optimizer`: the controller dispatches that after the commit, report-only. **Music C tasks** (`src/music_data.c`, `src/music_data.h`, or any new song `.c`) go to `music-expert` the same way.
