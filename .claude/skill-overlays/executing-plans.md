...
- **The plan's embedded hard-gate steps are mandatory, not advisory.** For any task touching `src/*.c` or `src/*.h`, every gate the plan lists must have **fired and reported** before the task is complete. `bank-pre-write` fires automatically as a hook (PreToolUse on `src/*` writes); `post-build-gates` fires automatically after a build (PostToolUse) — do not invoke them by hand; **read their output** and treat any FAIL as blocking. `gbdk-expert` and `gb-c-optimizer` are agent dispatches and must actually be dispatched.
