...
1. Headers: `gb/gb.h` (core), `gb/cgb.h` (CGB palettes), `gb/hardware.h` (raw regs), `stdio.h`
   for `printf`
2. Banking (bank pragma vs `bank-manifest.json`, `SET_BANK`/`SWITCH_ROM` placement, `BANKED` on
   cross-bank calls) → the `bank-pre-write` gate owns these rules and fires automatically on write
3. For everything else — deep hardware specs (OAM, palettes, PPU modes, interrupts), the VBlank
