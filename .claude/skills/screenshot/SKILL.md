...
Use when the symptom is a wrong WRAM value, not a visual glitch — faster than launching
Emulicious. Pattern (export debug globals, grep the map, read via PyBoy): see
`references/wram-read.md`. For a full headless diagnosis — navigate, read, iterate hypotheses —
dispatch the `pyboy-debug` agent instead of scripting it here.
