...
> (MIT, see `LICENSE`). Source: https://github.com/danyuchn/asd-ste100-skill
> Copied verbatim except the frontmatter `name:` field, which carries this
> directory's name so the skill registers.
...
Nuke Raider adopts ASD-STE100 as its writing standard. Three project rules apply.

### Bound surfaces

The standard binds:

- Factory-authored GitHub text: the run issue, the spec-issue comment, and the pull request body.
- PRD issue bodies.
- Factory plan documents.
- Repository documentation.

Game text is exempt. Dialog, mission briefings, NPC voice and on-screen strings keep their own
voice, and `docs/STORY_BIBLE.md` stays the authority for all of them.

Verbatim tool output is exempt. A `failure` message that quotes `spec_lint.py` output or `make`
output keeps the exact original text. Simplification of quoted output falsifies it.

### CONTEXT.md outranks this skill

`CONTEXT.md` is the project glossary. When it bans a word in an `_Avoid_` list, the ban wins —
even when the banned word is the plainer word. ASD-STE100 asks for the plainest word available;
this project asks for the one word each meaning owns. Where the two disagree, `CONTEXT.md` is
right. `tools/ste_lint.py` enforces this: an ASD-STE100 rule finding only reports, and a
banned-synonym hit exits 1 when you lint a file path or an issue body. The `--all` sweep reports
both classes and exits 0, because the glossary bans words this repository cannot write around.

### Forward only

The standard binds a document when an author creates it or edits it substantially. No existing
document is rewritten to satisfy it. `tools/ste_baseline.json` records the findings the tree
already carried, so `python tools/ste_lint.py --all` reports new prose and stays quiet about old
prose.
