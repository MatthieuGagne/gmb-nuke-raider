...
- **Agents** — `.omp/agents/*.md`, generated by `tools/sync_agents.py`. omp
  deliberately skips `.claude/agents` and `.pi/agents` because their frontmatter
  is not the omp task-agent contract, so the seven agents are mirrored here on
  every commit (`.githooks/pre-commit` runs the sync and stages the result).
  Frontmatter is translated to omp's dialect (`model` as `@default`/`@smol`,
  lowercase tool names), the body is copied verbatim from
  `.claude/agents/<name>.md`, and mirrors whose canonical source is gone are
  deleted — so the persona text lives in exactly one place. Do not hand-edit
  `.omp/agents/*.md`: edit the canonical file and the mirror regenerates on
  commit. Dispatch them with the `task` tool.
