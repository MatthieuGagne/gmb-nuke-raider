...
- **`headless-debug`** — Autonomous 5-step debug loop using PyBoy headless emulator — boot ROM, simulate inputs, read WRAM debug log, hypothesize, fix, verify. **Explicit opt-in only** — invoke when user says "use headless" or similar; requires `make build-debug` + `tests/integration/helpers.py`. When a scenario is generally reusable, promote it into `tests/integration/test_regression.py` (extend, don't duplicate).
...
**Integration regression suite:** `tests/integration/test_regression.py` covers the full state traversal (title → overmap → hub → overmap → playing → game_over → title). Run with `make test-integration` (builds debug ROM first). **Update this test whenever:** a game state is added or removed, an overmap node position changes, a state transition trigger changes, or `hp`/damage logic changes. Always ask the user before modifying or adding integration tests. New scenarios go in `test_regression.py` as additional test functions — do not create separate files unless the scenario is completely independent.
**Static WRAM symbol lookup:** SDCC does not export `static` file-scope variable names to the `.map` file. Use `find_wram_read_in_fn(noi_path, rom_path, "_getter_fn")` (in `helpers.py`) to locate static WRAM variables by disassembling their getter function from the ROM binary. `find_wram_sym_from_map` only works for non-static globals.
