...
| Race state | `src/race_state.c/.h` | Unified per-slot checkpoint/lap tracking for all racers (slot 0 = enemy, slot 1 = player); AABB+direction CP detection, lap advance gate, 3-level position ranking (laps → CPs → Manhattan tiebreaker); replaces the old `checkpoint.c` + `lap.c` split |
