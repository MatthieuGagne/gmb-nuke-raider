...
| Loadout | `src/loadout.c/.h` | Per-field loadout config (car, armor, weapon1, weapon2); persists across menu re-entries; initialized at boot. Tier-1 options (HEAVY armor, LASER, MINE) start **locked** and are hidden in prerace until unlocked at a hub vendor; CAR is never gated. HEAVY armor (#423) reduces all incoming damage by a flat `ARMOR_HEAVY_REDUCTION` (2 HP) per hit, floored at 1 HP (never full immunity — wall scrapes and rams still chip HP); LIGHT is unchanged. The tier is read once at race start (`state_playing` enter) and applied inside `damage_apply`; healing, i-frames, and max HP are unaffected. LASER primary (#424) deals `WEAPON1_LASER_DAMAGE` (2 HP) per bullet hit vs CANNON's 1 — a racer/patrol (HP 5) dies in 3 hits instead of 5, a turret (HP 1) still dies in 1; the per-hit damage is resolved once at race start (`state_playing` enter → `projectile_set_weapon1_damage`) and enemy bullets are unaffected |
