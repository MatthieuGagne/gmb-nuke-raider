...
void test_turret_pool_empty_after_init(void) {
    TEST_ASSERT_EQUAL_UINT8(0u, turret_count_active());
}

void test_turret_spawn_and_active(void) {
    turret_spawn(10u, 20u);   /* tile coords */
    TEST_ASSERT_EQUAL_UINT8(1u, turret_count_active());
}

void test_turret_blocks_tile_active(void) {
    turret_spawn(10u, 20u);
    TEST_ASSERT_EQUAL_UINT8(1u, turret_blocks_tile(10u, 20u));
}

void test_turret_blocks_tile_inactive(void) {
    /* No spawn — tile should not be blocked */
    TEST_ASSERT_EQUAL_UINT8(0u, turret_blocks_tile(10u, 20u));
}

void test_turret_blocks_tile_wrong_position(void) {
    turret_spawn(10u, 20u);
    TEST_ASSERT_EQUAL_UINT8(0u, turret_blocks_tile(5u, 5u));
}
...
void test_turret_spawn_timer_is_wind_up(void) {
    turret_spawn(10u, 20u);
    TEST_ASSERT_EQUAL_UINT8(TURRET_WIND_UP, turret_get_timer(0u));
}

void test_turret_three_turrets_timer_at_wind_up(void) {
    turret_spawn(4u, 22u);
    turret_spawn(15u, 47u);
    turret_spawn(5u, 72u);
    TEST_ASSERT_EQUAL_UINT8(TURRET_WIND_UP, turret_get_timer(0u));
    TEST_ASSERT_EQUAL_UINT8(TURRET_WIND_UP, turret_get_timer(1u));
    TEST_ASSERT_EQUAL_UINT8(TURRET_WIND_UP, turret_get_timer(2u));
}

void test_turret_timer_does_not_fire_during_wind_up(void) {
    turret_spawn(10u, 20u);
    TEST_ASSERT_EQUAL_UINT8(TURRET_WIND_UP, turret_get_timer(0u));
    TEST_ASSERT_EQUAL_UINT8(1u, turret_count_active());
}
...
void test_turret_visible_when_on_screen(void) {
    /* ty=10: world_y=80. cam_y=0 → vis_y=80-0+16=96 ∈ [0,160) ✓
     * tx=10: world_oam_x=88. cam_x=0 → vis_x=88 ∈ [0,168) ✓ */
    cam_y = 0u;
    cam_x = 0u;
    turret_spawn(10u, 10u);
    TEST_ASSERT_EQUAL_UINT8(1u, turret_is_screen_visible(0u));
}

void test_turret_not_visible_when_below_screen(void) {
    /* ty=20: world_y=160. cam_y=0 → vis_y=160-0+16=176 ≥ 160 → off screen */
    cam_y = 0u;
    cam_x = 0u;
    turret_spawn(10u, 20u);
    TEST_ASSERT_EQUAL_UINT8(0u, turret_is_screen_visible(0u));
}

void test_turret_not_visible_when_above_screen(void) {
...
void test_turret_not_visible_when_off_screen_right(void) {
    /* tx=22: oam_x=22*8+8=184. cam_x=0 → vis_x=184 ≥ 168 → off screen */
    cam_y = 0u;
    cam_x = 0u;
    turret_spawn(22u, 10u);
    TEST_ASSERT_EQUAL_UINT8(0u, turret_is_screen_visible(0u));
}

void test_turret_spawn_sets_type_turret(void) {
    turret_spawn(5u, 5u);
    TEST_ASSERT_EQUAL_UINT8(NPC_TYPE_TURRET, turret_get_type(0u));
}

void test_turret_spawn_sets_dir_none(void) {
    turret_spawn(5u, 5u);
    TEST_ASSERT_EQUAL_UINT8(DIR_NONE, turret_get_dir(0u));
}
...
int main(void) {
    ...
    RUN_TEST(test_turret_direction_to_player_right);
    RUN_TEST(test_turret_direction_to_player_down);
    RUN_TEST(test_turret_direction_to_player_down_right);
    RUN_TEST(test_turret_direction_to_player_up_left);
    RUN_TEST(test_turret_direction_nne);
    RUN_TEST(test_turret_direction_ene);
    RUN_TEST(test_turret_direction_ese);
    RUN_TEST(test_turret_direction_sse);
    RUN_TEST(test_turret_direction_ssw);
    RUN_TEST(test_turret_direction_wsw);
    RUN_TEST(test_turret_direction_wnw);
    RUN_TEST(test_turret_direction_nnw);
    RUN_TEST(test_turret_spawn_timer_is_wind_up);
    RUN_TEST(test_turret_three_turrets_timer_at_wind_up);
    RUN_TEST(test_turret_timer_does_not_fire_during_wind_up);
    RUN_TEST(test_turret_visible_when_on_screen);
    RUN_TEST(test_turret_not_visible_when_below_screen);
    RUN_TEST(test_turret_not_visible_when_above_screen);
    RUN_TEST(test_turret_not_visible_when_off_screen_right);
    RUN_TEST(test_turret_spawn_sets_type_turret);
