...
EXEMPT_GENERATED = (
    ...
    'src/track_tiles.c',
    'src/turret_sprite.c',
    # Not generated by any of the tools above: a hUGETracker/hUGEDriver song
    # data table, data only, no functions (lizard reports zero for it).
    'src/music_data.c',
)
...
def main(argv=None):
    ...
    try:
        ...
        if args.files:
            normalised = [os.path.normpath(f).replace('\\', '/') for f in files]
            kept = [f for f in normalised if f.startswith('src/') and f.endswith('.c')]
            discarded = [orig for orig, norm in zip(files, normalised)
                         if not (norm.startswith('src/') and norm.endswith('.c'))]
            if discarded:
                sys.stderr.write(
                    "crap_score: --files entries not recognised as in-scope src/*.c paths, "
